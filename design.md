...
2. プロフィールがなければ `profile` を表示する
3. プロフィール保存後は `lobby` へ遷移する
4. `lobby` でバーコードを読み込み、キャラクター準備を行う
5. キャラクター準備後に部屋作成または部屋参加を実行できる
6. 部屋作成または参加後は `room` へ遷移する
7. Firebase上で両者の準備が揃い、対戦開始したら `battle` へ遷移する
8. 勝敗確定後は `result` へ遷移し、ランキング保存結果を表示する
9. 結果画面から `ranking` または `lobby` へ遷移できる
10. `ranking` から `lobby` へ戻れる

### コンポーネント分割

- `ProfileScreen`: タイトル、ユーザー名入力、保存
- `LobbyScreen`: 保存済みユーザー名、バーコード読み込み、部屋作成、部屋参加、ランキング導線
...
Phase 5では部屋作成後の準備画面でバーコードを入力していた。Phase 6ではロビーで自分のバーコードを読み込み、キャラクターを作成してから部屋作成または参加へ進む。
