...
Phase 7の画面は、ブラウザ全体をゲーム画面として使う。ページ全体を囲う大きな枠や筐体風の外枠は使わず、各画面の主役になる要素だけに枠、カード、ボタンの強調を使う。

- 画面の最小高さはブラウザ高に合わせ、低い端末ではスクロールを許可する
- スマホ縦画面を基準にし、タブレット/PCでは横幅ではなく余白を増やす
- 主コンテンツは用途に応じて `narrow`、`medium`、`wide` の最大幅に収める
- フォーム、START、コマンドなどの主要操作は44px以上のタップ領域を維持する
- タイトル、結果、ランキングは中央寄せの縦レイアウトにする
- ユーザー名入力、キャラクター準備、ロビーは中央のコンテンツカラムにする
- 部屋待機と対戦は、狭い画面では縦積み、広い画面では情報量に応じて横展開する
