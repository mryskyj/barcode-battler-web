...
- `character`: カメラ読み取りを主導線にし、画面遷移直後からカメラを表示する。バーコード値の初期状態は空にし、読み取れた場合だけ値を表示する。手入力は「数字を直接入力」の小さな補助操作として扱う。中央にはバーコードカードとスキャン光を置き、生成後のキャラクターカードやステータス表示を強調する
- `lobby`: 準備済みキャラクターを表示し、部屋作成/参加を対戦準備メニューとして整理する。ランキングはサブ導線にする
- `room`: 部屋ID、接続状態、自分と相手の準備状態を、待機中の対戦カードとして見せる
